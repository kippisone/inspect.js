...
const InspectionError = require('./errors/InspectionError');
...
module.exports = (Inspect) => {
  ...
  /**
   * Inspect whether a file content is equal with a certain content
   *
   * @method fileEquals
   * @version v1.10.0
   *
   ...
   *
   * @example {js}
   * inspect('/tmp/test.json').fileEquals('Foo');
   *
   * @chainable
   * @returns {object} Returns `this` value
   */
  Inspect.prototype.fileEquals = function(match, opt, message) {
    opt = opt || {}
    if (typeof opt === 'string') {
      message = opt
      opt = {}
    ...
    if (content !== match) {
      const splicedString = match.length > STR_MAX_LEN ? match.substr(0, STR_MAX_LEN) + '...' : match
      const splicedContent = content.length > STR_MAX_LEN ? content.substr(0, STR_MAX_LEN) + '...' : content
      throw new InspectionError('File ' + this.inspectValue + ' is not equal with match!', splicedContent, splicedString);
    }
