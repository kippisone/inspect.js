...
function getFileType() {
  ...
  try {
    stat = fs.lstatSync(this.inspectValue);
...
module.exports = (Inspect) => {
  ...
  Inspect.prototype.doesFileExists = function(message) {
    try {
      fs.accessSync(this.inspectValue)
    } catch (err) {
      throw new InspectionError('File ' + this.inspectValue + ' does not exists!');
    }
  ...
  Inspect.prototype.isFile = function(message) {
    const fileType = getFileType(this.inspectValue);
  ...
  /**
   * Inspects whether a file exists and it is not a file
   *
   * @method isNotFile
   * @version v1.6.0
   *
  ...
  Inspect.prototype.isDirectory = function(message) {
    ...
    if (!fileType) {
      throw new InspectionError('Dir ' + this.inspectValue + ' does not exists!');
    }
  ...
  /**
   * Inspects whether a file exists and it is not a directory
   *
